...
import CallDialog from "@/components/custom/CallDialog";
import { useCall } from "@/hooks/call";

function Home() {
  const { user } = useSelector((state) => state.auth);
  const { activeUsers } = useSelector((state) => state.user);
  const { callUser } = useSelector((state) => state.call);

  const { open, setOpen, handleOpen, handleClose } = useCall();
    ...
    <>
      <div className="h-full w-full flex  items-center justify-center overflow-hidden">
        <div className="h-full w-full sm:w-8/12 flex flex-col items-center bg-slate-200">
          ...
          <div className="w-ful flex items-center px-4 bg-slate-200">
            <TabsList className={"px-2 mb-2 sm:w-[240px] w-full bg-slate-300"}>
